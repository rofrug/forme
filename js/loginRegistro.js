document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("input", function () {
      ...
      document
        ...
        .classList.toggle("text-teal-600", val.length >= 8);
      document
        .getElementById("req-mayus")
        .classList.toggle("text-teal-600", /[A-Z]/.test(val));
      document
        .getElementById("req-num")
        .classList.toggle("text-teal-600", /\d/.test(val));
    });
  ...
  document
    ...
    .addEventListener("input", function () {
      const val = this.value;
      document
        .getElementById("r-req-length")
        .classList.toggle("text-teal-600", val.length >= 8);
      document
        .getElementById("r-req-mayus")
        .classList.toggle("text-teal-600", /[A-Z]/.test(val));
      document
        .getElementById("r-req-num")
        .classList.toggle("text-teal-600", /\d/.test(val));
      actualizarCoincidencia();
    });

  document
    .getElementById("registro-confirm")
    .addEventListener("input", actualizarCoincidencia);
  ...
  function actualizarCoincidencia() {
    ...
    document
      .getElementById("r-req-match")
      .classList.toggle("text-teal-600", pass === conf && pass !== "");
  }
});
